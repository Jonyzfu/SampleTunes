...
class SearchViewController: UIViewController {
            ...
            if let data = data, response = try NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions(rawValue: 0)) as? [String: AnyObject] {
                ...
                if let array: AnyObject = response["results"] {
                    for trackDictionary in array as! [AnyObject] {
                        if let trackDictionary = trackDictionary as? [String: AnyObject], previewUrl = trackDictionary["previewUrl"] as? String {
    ...
    func playDownload(track: Track) {
        if let urlString = track.previewUrl, url = localFilePathForUrl(urlString) {
            ...
            self.presentViewController(audioPlayerViewController, animated: true, completion: { () -> Void in
    ...
    // This method generate a permanent local file path to save a track to by appending
